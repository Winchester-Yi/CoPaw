...
批模式的物理定时器是唤醒源，独立暂停由 Scheduler 门控，所以暂停时仍可能收到被明确忽略的批回调。

旧逻辑可能已经暂停物理批定时器。SWE 的 `PUT /api/cron/jobs/{job_id}/batch-dispatch/run-state` 收到 paused=false 和当前 expected_version 后，先恢复父任务 meta 中的 batch_dispatch_external_job_id，再修改独立状态。即使控制记录本来是运行，也可用该请求定向修复物理定时器；不操作 external_job_id 对应的普通定时器，不遍历其他租户。

批定时器 ID 缺失或外部 Scheduler 不可用时，操作明确失败，不把控制状态改为运行。应修复该父任务的批调度配置，不能批量 resume 普通任务作为补救。
...
- 暂停期间授权失败任务重试只入队并提示等待恢复；不绕过修复确认、一次额外尝试授权或 Worker 限制。

...
**管理端活跃模型选择**:
模型管理界面中点击远程 Provider 的具体模型条目主体即可将可用模型设为当前租户活跃模型并立即持久化，不额外提供“设为激活模型”按钮或独立暂存保存步骤；条目内的设置图标/配置入口用于打开该模型的详细设置。它与聊天活跃模型选择共享同一活跃模型状态。
_Avoid_: 管理端独立默认模型、Provider 级活跃模型、独立激活按钮、模型条目仅展示静态信息、失效的本地模型管理入口

**管理端远程模型范围**:
当前模型管理界面只展示和选择远程 Provider 的模型；不再提供顶部活跃模型选择器，远程模型条目的即时激活是唯一入口。本地 Provider 卡片及本地模型均不在管理端展示。后端保留的本地模型兼容接口不构成管理端可用能力。
_Avoid_: 隐藏卡片但保留顶部本地选项、并行的暂存选择器、不可用本地占位卡片、把兼容接口当作可操作功能

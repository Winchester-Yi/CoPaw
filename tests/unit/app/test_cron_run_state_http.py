...
@pytest.mark.parametrize(
    "outcome,status",
    [("ok", 200), ("conflict", 409), ("timeout", 502), ("no-token", 200)],
)
async def test_http_proxy_keeps_parent_tenant_separate_from_operator(
    monkeypatch, outcome, status, method
):
    ...
    if status == 200:
        assert (await client.request_run_state(job, "admin", method=method))[
            "paused"
        ] is True
    else:
        with pytest.raises(HTTPException) as exc:
            await client.request_run_state(job, "admin", method=method)
